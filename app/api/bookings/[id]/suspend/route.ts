...
export async function POST(
  ...
  try {
    ...
    await db.collection('bookings').doc(bookingId).update({ status: 'suspended' });

    await writeAuditLog(admin.email, 'booking_suspended', bookingId, 'booking', 'Booking suspended by admin');
